      ...
      @if (!store.file()) {
        ...
        <div class="file-preview">
          ...
          <div style="flex:1;min-width:0;">
            ...
            <div class="file-meta">
              {{ store.file()!.pageCount }} sayfa · {{ store.file()!.size | fileSize }}
            </div>
          </div>
        ...
        <div class="tab-bar">
          <button
            class="tab"
            [class.active]="store.mode() === 'range'"
            (click)="store.setMode('range')"
          >
            Aralık
          </button>
          <button
            class="tab"
            [class.active]="store.mode() === 'burst'"
            (click)="store.setMode('burst')"
          >
            Her Sayfa Ayrı
          </button>
          <button
          ...
          @case ('range') {
            <div class="section">
              <div class="input-group">
                <label class="input-label" for="rangeInput">Sayfa Aralıkları</label>
                <input
                  id="rangeInput"
                  class="field"
                  type="text"
                  placeholder="örn: 1, 3-5, 10-son"
                  [ngModel]="store.rangeInput()"
                  (ngModelChange)="store.setRangeInput($event)"
                />
          ...
          @case ('burst') {
            <div class="section">
              <div class="info-box">
                ...
                Her sayfa ayrı bir PDF dosyası olarak kaydedilecek (toplam
                {{ store.file()!.pageCount }} dosya).
              </div>
            </div>
          }
          @case ('fixed') {
            <div class="section">
              <div class="input-group">
                <label class="input-label" for="fixedSizeInput">Her Dosya İçin Sayfa Sayısı</label>
                <input
                  id="fixedSizeInput"
                  class="field"
                  type="number"
                  min="1"
                  [max]="store.file()!.pageCount"
                  [ngModel]="store.fixedSize()"
                  (ngModelChange)="store.setFixedSize($event)"
                />
              </div>
            </div>
          }
        ...
        <div class="prefix-row">
          <div class="input-group">
            <label class="input-label" for="prefixInput">Çıktı Dosya Adı Öneki</label>
            <input
              id="prefixInput"
              class="field"
              type="text"
              [ngModel]="store.filenamePrefix()"
              (ngModelChange)="store.setFilenamePrefix($event)"
            />
          </div>
        </div>
        ...
        <div class="actions">
          <button
            class="btn-accent"
            [disabled]="store.isProcessing() || previewFiles().length === 0"
            (click)="store.split()"
          >
