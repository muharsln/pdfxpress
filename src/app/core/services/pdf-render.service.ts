...
export interface IPdfRenderService {
  ...
  renderAllPages(file: File, width: number, format?: string, quality?: number, onProgress?: (current: number, total: number) => void): Promise<Map<number, string>>;
  cancelAll(): void;
...
export class PdfRenderService implements IPdfRenderService {
  ...
  async renderPage(file: File, pageNumber: number, width: number, format: string = 'jpeg', quality: number = 0.8): Promise<string> {
