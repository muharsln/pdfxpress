...
@Component({
  ...
  template: `
    <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 backdrop-blur-sm">
      <div
        class="bg-[var(--color-surface)] rounded-2xl p-8 flex flex-col items-center gap-4 shadow-2xl"
      >
        <div class="relative w-24 h-24">
          ...
          <span
            class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-xl font-semibold text-[var(--color-on-surface)]"
            >{{ progress() }}%</span
          >
        </div>
